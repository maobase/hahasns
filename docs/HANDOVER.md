...
- **存量文件提示（v5.74，v5.77 改成看引用）**：`storage status` 返回 `localFiles` / `localFilesCapped`
  （数 uploads 目录里的文件，跳过点开头的探针残留与子目录，上限 10000），
  再加 `localReferenced` / `localOrphans` / `localRefScanned`——**面板与自检只对 `localReferenced` 报警**。
  只数文件个数会把孤儿（删过的帖子、测试残留）也算成待迁，env2 实测报「还剩 2 个」而 dry-run 待重写 0 个。
  判定层是纯函数 `storage/uploads-refs.ts`（`MEDIA_REF_COLUMNS` + 路径解析 + 分堆），SQL 在
  `StorageService.splitLocalByReference()` 里跑，只在 `driver === 's3'` 时才扫。
  **`MEDIA_REF_COLUMNS` 必须与迁移脚本的 `targets` 同步**——`test/uploads-refs.test.ts` 有一条用例
  直接读脚本源码逐项比对，漏改会红。扫库整体失败（缺表、权限不足）时退化成「全都算被引用」
  并把 `localRefScanned` 置 false，页面注明这次没对上引用：宁可多提醒，不能把在用的文件说成不用迁。
...
- **「存量文件没迁走」这条曾经偏保守，v5.77 已修**：它原先数的是 uploads 目录里的文件数，
...
4. **对象删除不清理存储对象**：`StorageService.delete` 目前无调用方（预留），
   ...
   **v5.77 已把扫描的判定层做出来了**（`uploads-refs.ts` 的 `MEDIA_REF_COLUMNS` + `extractUploadKeys`
   已能列出「库里还引用哪些 key」），要做桶内孤儿扫描时复用它列引用、再与 `ListObjectsV2` 求差即可。
5. **存量迁移仍是手工跑脚本**：v5.74 起会提示「还剩 N 个没迁走」，但迁移本身要 SSH 上服务器执行；
   后台一键迁移需要长任务 + 进度回传（当前没有任务队列），属较大改动。
   同理「测试连接」只验探针对象，不校验存量文件是否真的都能读到。
6. **引用扫描只扫顶层文件与 7 处库列**：子目录不递归、文件数上限 10000（超出显示 `N+`），
   引用侧覆盖 `MEDIA_REF_COLUMNS` 那 7 列（与迁移脚本同步）。落在这之外的引用（比如将来
   新表存了 `/uploads/` 路径而两处都没加）会被算成孤儿——**加存媒体路径的表时两处一起加**。
   要精确统计仍以迁移脚本的 dry-run 输出为准。
